...
class Crawler:
    ...
    async def get_summaries_from(self, links: list[str]) -> list[ExtractSchema]:
        ...
        stories: list[ExtractSchema] = []
        async with self.crawler as crawler:
            results = await crawler.arun_many(urls=links, config=crawl_config)
            ...
            for result in results:
                if result.success:
                    try:
                        ...
                        stories.append(ExtractSchema(**json_result[0]))
                    except (ValidationError, IndexError) as e:
                        print("Error parsing content for:\n", result.extracted_content)
                else:
